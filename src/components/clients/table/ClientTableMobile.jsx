...
import { TrendingUp, TrendingDown, MoreVertical, FileText, ChevronDown, ChevronUp } from "lucide-react";
    ...
    <div className="sm:hidden mt-4 border border-gray-200 rounded-xl bg-white shadow-sm overflow-visible">
      ...
      <div className="grid grid-cols-[1.5fr_2fr_0.5fr] bg-gray-100 text-[12px] font-semibold text-gray-700 border-b border-gray-200">
        ...
        <div className="px-3 py-2 flex flex-col text-right leading-tight">
          <span className="text-gray-800 text-[12px] font-semibold">{headerActual}</span>
      ...
      {clients.map((client) => {
        ...
        const totalActual =
          ...
          client.revenueCurrentYear?.reduce((sum, m) => sum + (m.total || 0), 0) ??
          0;
        const totalAnterior =
          client.totalLastYear ??
          client.revenueLastYear?.reduce((sum, m) => sum + (m.total || 0), 0) ??
          0;
        const crecimiento = totalAnterior > 0 ? ((totalActual - totalAnterior) / totalAnterior) * 100 : 0;
        const colorCrec = crecimiento > 0 ? "text-green-600" : crecimiento < 0 ? "text-red-500" : "text-gray-400";
        ...
        return (
          <div key={client.id}>
            ...
            >
              {/* Chevron + Nombre + Badge */}
              <div className="px-3 py-2 text-gray-900 font-medium flex items-center gap-2">
                {isOpen ? (
                ...
                {client.segment && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${getSegmentColor(client.segment)}`}>
                    {client.segment}
                  </span>
                )}
              ...
              <div className="px-3 py-2 text-right leading-tight">
                ...
                <div className="text-[11px] text-gray-500">{formatters.currency(totalAnterior)}</div>
                <div className={`flex items-center justify-end gap-1 text-[10px] ${colorCrec}`}>
                  {crecimiento >= 0 ? <TrendingUp size={11} className={`${colorCrec}`} /> : <TrendingDown size={11} className={`${colorCrec}`} />}
              ...
              <button
                ...
                aria-label={`Acciones cliente ${client.name}`}
              >
            ...
            {isOpen && (
              <div className="bg-blue-50 px-3 py-3 text-sm text-gray-700 border-t border-blue-100">
                ...
                {DETAIL_FIELDS.map(({ label, key }) => {
                  ...
                  const crecimientoFam = anterior > 0 ? ((actual - anterior) / anterior) * 100 : 0;
                  const color = crecimientoFam > 0 ? "text-green-600" : crecimientoFam < 0 ? "text-red-500" : "text-gray-400";

                  return (
                    <div key={key} className="grid grid-cols-[1.5fr_2fr_2fr] py-1 border-b border-blue-100 last:border-none">
                      <span className="truncate text-[12px] text-gray-700">{label}</span>

                      <span className="text-right text-[11px] text-gray-500">{formatters.currency(anterior)}</span>

                      <div className="flex flex-col items-end leading-tight">
                        <span className="text-[12px] font-semibold text-gray-800">{formatters.currency(actual)}</span>

                        <div className={`flex items-center justify-end gap-1 text-[10px] ${color}`}>
                          {crecimientoFam >= 0 ? <TrendingUp size={11} className={`${color}`} /> : <TrendingDown size={11} className={`${color}`} />}
