...
export function RouteClientCard({ client, onStartRoute }) {
  ...
  const monthlyOrders = orders.filter(
    (o) => dayjs(o.date).month() + 1 === currentMonth
  );
  const ticketMedio =
    monthlyOrders.length > 0
      ? Math.round(currentMonthTotal / monthlyOrders.length)
      : 0;

  const isInactive = monthlyOrders.length === 0 && monthlyGrowth < -10;
  const riskLevel =
    isInactive || growth < -10 ? "Alto" : growth < 0 ? "Medio" : "Bajo";
    ...
    <>
      ...
      />
        ...
        {currentFamilies.length > 0 && (
          <RouteClientFamilies families={currentFamilies} />
        )}
    </>
  );
}
